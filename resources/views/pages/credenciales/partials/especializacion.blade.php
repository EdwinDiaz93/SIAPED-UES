                    ...
                    @forelse ($this->especializaciones as $r)
                        <tr class="hover:bg-surface-alt/50">
                            ...
                            <td class="p-3 text-center whitespace-nowrap">
                                @if ($esAdmin)
                                    @if ($r->estado !== 'aprobado')
                                        <button wire:click="aprobarCredencial('especializacion', {{ $r->id }})" class="text-green-600 hover:text-green-800 text-xs font-medium mr-1 cursor-pointer">Aprobar</button>
                                    @endif
                                    @if ($r->estado !== 'rechazado')
                                        <button wire:click="rechazarCredencial('especializacion', {{ $r->id }})" class="text-red-500 hover:text-red-700 text-xs font-medium cursor-pointer">Rechazar</button>
                                    @endif
                                @else
                                    @if ($r->estado !== 'aprobado')
                                        <button wire:click="editarEspecializacion({{ $r->id }})" class="text-blue-500 hover:text-blue-700 mr-2 cursor-pointer">✏</button>
                                        <button type="button" x-on:click="confirmAction('¿Eliminar?', () => $wire.eliminarEspecializacion({{ $r->id }}))" class="text-red-500 hover:text-red-700 cursor-pointer">✕</button>
                                    @else
