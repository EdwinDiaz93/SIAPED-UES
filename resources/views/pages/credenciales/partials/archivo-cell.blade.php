...
@if ($r->archivo_path)
    <a href="{{ Storage::disk('public')->url($r->archivo_path) }}"
       target="_blank"
       title="{{ $r->archivo_descripcion ?? 'Ver archivo' }}"
       class="inline-flex items-center gap-1 text-xs text-ues underline hover:opacity-80">
        @php $ext = pathinfo($r->archivo_path, PATHINFO_EXTENSION); @endphp
        @if ($ext === 'pdf')
        ...
        @endif
        {{ Str::limit($r->archivo_descripcion ?? 'Ver', 20) }}
    </a>
@else
